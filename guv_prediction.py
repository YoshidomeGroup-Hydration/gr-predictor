...
class Prediction():
    ...
    def __init__(self, pdb_dir, dx_dir, split_center=16, split_size=48, guv_dir=None, pred_area_center=np.array([None]), pred_area_range=-1):
        ...
        if self.analysis_dir == None:
            # オリジナルの座標基準値を生成
            min_ax = np.array([self.Atom_x.min(), self.Atom_y.min(), self.Atom_z.min()])-14.
            max_ax = np.array([self.Atom_x.max(), self.Atom_y.max(), self.Atom_z.max()])+14.
            self.R_Grid = min_ax
        ...
        if pred_area_center[0] != None:
            ...
            self.pred_area_range = pred_area_range
            # 局所予測
            self.predicter_local(model_dir = "D:/LAB/new/resUnet_ALSP/CheckPoint_1_198.h5")
        # 通常ver
        else:
            # 全体ボクセル化
            ...
            self.predicter_normal(model_dir = "D:/LAB/new/resUnet_ALSP/CheckPoint_1_198.h5")
