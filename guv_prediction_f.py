...
class Prediction():
    ...
    def __init__(self, pdb_dir, dx_dir, split_center=16, split_size=48, guv_dir=None, pred_area_center=np.array([None]), pred_area_range=-1, model_dir="model_1.h5"):
        ...
        self.Atom_x, self.Atom_y, self.Atom_z, self.atype, self.aminotype = self.ReadPDB(pdb_dir)
    ...
    def ReadPDB(self,path_pdb):
        ...
        dict_ = {"H":"1", "C":"6", "N":"7", "O":"8", "S":"16"}
        for key, value in dict_.items():
            atom_type = [c.replace(key, value) for c in atom_type]
        atom_type = [int(s) for s in atom_type]

        return np.array(atom_x), np.array(atom_y), np.array(atom_z), np.array(atom_type), amino_type
    ...
    def define_hako(self, offset): 
        ...
        print("center:{}   split_size:{}   arr.shape: {}".format(center,split_size, arr.shape))
        for xyz in range(3):
            size,cnt = 0,0
            # 予測領域（＝箱の中心×n）の最小の大きさを決定する
            # 3DRISMのボックスサイズから両端x/2だけ削った領域を最小の予測領域とする
            # デフォルトはx=24 ：　両端から6A削る
            print(arr.shape[xyz+1], offset, size, edge)
